...
int Event_Counter() {
	string path = "/home/dylan/Research/AMPT_Test_Data/7GeV/";
	vector<string> in_files = get_files_in_dir(path, "root", "path");
	TH1I *num_events = new TH1I("Num_Events", "Number of AMPT Events", 101, -0.5, 1000.5);
	int num_quit = 0;
	for(string file:in_files) {
		TFile *f = new TFile(file.data(), "READ");
		TTree *tree = (TTree*)f->Get("tree");
		num_events->Fill(tree->GetEntries());
		if(tree->GetEntries() < 1000) { cout << "Entries: " << tree->GetEntries() << endl; num_quit++; }
		f->Close();
		delete f;
	}
